...
const actions = {
    updateSessionList({ commit, state, rootState }) {
        return Skrumble.getUsers().then(users => {
            var list = [];
            users.forEach(user => {
                if (user.Address != rootState.user.address) {
                    try {
                        var key = Skrumble.ecdh(rootState.user.privateKey, user.PublicKey).toString("hex");
                        list.push({
    ...
    sendMessage({ commit, state }, content) {
        ...
        return {
            to: state.currentSessionAddress,
            content: Skrumble.encrypt(message.content, session.key)
        };
    },
    updateMessages({ commit, state, rootState }, messages) {
        messages.forEach(message => {
            ...
            temp.to = to;
            temp.content = Skrumble.decrypt(message.content, session.key)
